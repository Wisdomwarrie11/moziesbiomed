...
const HomeSkeleton = () => (
  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
    {[1, 2, 3].map((i) => (
      <div key={i} className="bg-white rounded-[50px] p-8 border border-gray-100 shadow-sm animate-pulse">
        <div className="aspect-[4/3] rounded-[35px] bg-gray-100 mb-8" />
        <div className="h-8 bg-gray-100 rounded-full w-3/4 mb-4 mx-auto" />
        <div className="h-4 bg-gray-100 rounded-full w-1/4 mb-4 mx-auto" />
        <div className="h-20 bg-gray-50 rounded-[22px] w-full" />
      </div>
    ))}
  </div>
);
...
const Home: React.FC = () => {
  const [featuredProducts, setFeaturedProducts] = useState<Product[]>(() => {
    const cached = localStorage.getItem('mozies_featured_cache');
    return cached ? JSON.parse(cached) : [];
  });
  const [categories, setCategories] = useState<CategoryItem[]>(() => {
    const cached = localStorage.getItem('mozies_categories_cache');
    return cached ? JSON.parse(cached) : [];
  });
  const [loading, setLoading] = useState(featuredProducts.length === 0);

  useEffect(() => {
    // Background listener for Categories
    const unsubCats = onSnapshot(collection(db, 'categories'), (snapshot) => {
      const cats = snapshot.docs.map(d => ({ id: d.id, ...d.data() } as CategoryItem));
      setCategories(cats);
      localStorage.setItem('mozies_categories_cache', JSON.stringify(cats));
    });

    // Background listener for Featured Products
    const unsubProds = onSnapshot(query(collection(db, 'products'), limit(3)), (snapshot) => {
      ...
      localStorage.setItem('mozies_featured_cache', JSON.stringify(prods));
      setLoading(false);
    });
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-6">
                  <div className="aspect-[4/5] rounded-[40px] overflow-hidden shadow-2xl border-4 border-white transform hover:rotate-2 transition-transform duration-500">
                    <img src="CT.jpeg" alt="Tech" className="w-full h-full object-cover" loading="lazy" />
                  </div>
                ...
                <div className="pt-12 space-y-6">
                  ...
                  <div className="aspect-[4/5] rounded-[40px] overflow-hidden shadow-2xl border-4 border-white transform hover:-rotate-2 transition-transform duration-500">
                    <img src="Endo.jpeg" alt="Tech" className="w-full h-full object-cover" loading="lazy" />
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                <p className="text-xl text-gray-600 font-medium leading-relaxed">
                  Moziesbiomed stands at the intersection of medical science and technical mastery. We don't just supply hardware; we architect operational efficiency.
                </p>
                <div className="grid grid-cols-2 gap-10 pt-4">
                  <div>
                    <h4 className="font-black text-gray-900 uppercase tracking-tight text-lg mb-2">Sustainable Design</h4>
                    <p className="text-sm text-gray-500 font-medium">Engineering approaches that maximize the lifecycle of high-value equipment.</p>
                  </div>
                  <div>
                    <h4 className="font-black text-gray-900 uppercase tracking-tight text-lg mb-2">Expert Synergy</h4>
                    <p className="text-sm text-gray-500 font-medium">Multidisciplinary teams tackling complex facility setups and maintenance.</p>
                  </div>
                </div>
              </div>
              <div className="pt-8">
                  ...
                  <div className="w-full aspect-[4/3] rounded-[35px] overflow-hidden mb-8 relative bg-gray-50 shadow-inner">
                    ...
                    <div className="absolute top-6 left-1/2 -translate-x-1/2">
                      <span className="bg-red-600/90 backdrop-blur-md text-white text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-xl whitespace-nowrap">
                        {categories.find(c => c.id === product.categoryId)?.name || 'In-Stock'}
                      </span>
                    </div>
                  </div>
